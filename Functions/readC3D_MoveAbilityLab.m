function [POINTdat,VideoFrameRate,ANALOGdat,AnalogFrameRate,ParameterGroup,HeaderGroup]...
    = readC3D_MoveAbilityLab(FullFileName)
...
% %FullFileName = 'C:\Users\Niklas\Documents\c3d\MC01S100W09.c3d';
ind = findstr(FullFileName,'\');

% har tagit bort "(FullFileName,'\');"
if ind > 0
...
if (isempty(key) | isempty(NrecordFirstParameterblock))
    fclose(fid)
    fid = -1
    return
end
...
if proctype == 2
    return                     % DEC VAX D floating point and VAX ordering
end
...
% if the scale value is positive the data type is integer
if Scale < 0
    DataTypeFloat = true;
else
    DataTypeFloat = false;
...
if DataTypeFloat
    ...
    reps = [int2str(4*NM),'*int16'];
    tmpMKR = fread(fid,4*NM*NVF,reps,2*NAPVF*NC);   % note two bytes per analog channel data skipped
end


% Read in analog data as repeated blocks
if (NC > 0)
    fseek(fid,(NrecordDataBlock-1)*512+2*4*NM,'bof');
    if DataTypeFloat
        ...
        fseek(fid,(NrecordDataBlock-1)*512+2*4*NM,'bof');
        reps = [int2str(NAPVF*NC),'*int16'];
        tmpANL = fread(fid,NAPVF*NC*NVF,reps,2*4*NM);   % note: two bytes per 3d data skipped
    end
end
...
if(NC > 0)
    ...
    ANL_offset = repmat(mean(ANALOGdat(:,1:30),2),1,NAPVF*NVF);
    ...
    SCALEmtx = repmat(SCALE,1,NAPVF*NVF);
